...
def xnor_select(arr, prc_1, prc_2)
    selected = []
    arr.each { |el| selected << el if prc_1.call(el) == prc_2.call(el) }
    selected
end
...
def selected_map!(arr, selector, mapper)
    arr.each_with_index do |el, i|
        arr[i] = mapper.call(el) if selector.call(el)
    end
    nil
end
...
def chain_map(n, procs)
    procs.inject(n) { |result, prc| prc.call(result) }
end
...
def proc_suffix(sentence, hash)
    ...
    words.each_with_index do |word, i|
        hash.each do |prc, suffix|
            new_words[i] += suffix if prc.call(word)
        end
    end
...
def procipher(sentence, hash)
    ...
    new_words = words.map do |word|
        new_word = word
        hash.each do |selector, mapper|
            new_word = mapper.call(new_word) if selector.call(word)
        end
        new_word
    end
...
def picky_procipher(sentence, hash)
    ...
    new_words = words.map do |word|
        new_word = word
        hash.reverse_each do |selector, mapper|
            new_word = mapper.call(word) if selector.call(word)
        end
        new_word
    end
