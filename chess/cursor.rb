...
class Cursor
  ...
  def read_char
    ...
    if input == "\e" then
      input << STDIN.read_nonblock(3) rescue nil # read_nonblock(maxlen) reads at most maxlen bytes from a data stream; it's nonblocking, meaning the method executes asynchronously;
      input << STDIN.read_nonblock(2) rescue nil # it raises an error if no data is available, hence the need for rescue
    end
