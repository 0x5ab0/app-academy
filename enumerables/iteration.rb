...
def substrings(string)
    ...
    string.each_char.with_index do |char_1, i|
        string.each_char.with_index do |char_2, j|
            substrings_array << string[i..j] if j >= i
        end
    end
...
def subwords(word, dictionary)
    valid_words = substrings(word).select do |valid_word|
        dictionary.include?(valid_word)
    end
...
# Don't use the built-in Array#map method.

def doubler(array)
    doubled = []

    array.each { |el| doubled << el * 2 }
